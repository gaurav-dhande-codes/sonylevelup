...
type UserStore interface {
	...
	// Returns the User model or an error if the user is not found or if an
	// internal error occurs.
	GetUser(userId int) (*model.User, error)

	// GetUserGameLibrary retrieves the game library for a given userId.
	// Returns a UserLibrary containing the list of owned games or an error if
	// the user is not found or if an internal error occurs.
	GetUserGameLibrary(userId int) (*model.UserLibrary, error)

	// GetUserGameAchievementCompletion retrieves the achievement completion
	// details for a specific user and game, identified by userId and gameId.
	// Returns a UserGameAchievementCompletion or an error if the user or game
	// is not found or if an internal error occurs.
